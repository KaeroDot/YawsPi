...
class yawpihw:
    ...
    def _init_vars(self):  # initialization of variables
        ...
        gv.hw = {
            # rpi revision:
            'RPiRevision' = ''
            # is hardware really connected?:
            'WithHW' = 0
            # number of stations:
            'StNo' = len(self.hwc['St'])
            # status (on/off) of water source:
            'SoStatus' = 0
            # status (on/off) of valves of stations:
            'StStatus' = [0] * len(self.hwc['St'])
            # status (on/off) of sensors (has sense only for grad sensors):
            'SeWLStatus' = [-1] * len(self.hwc['SeWL'])
            # water level of water source:
            'SoWL' = 0
            # water level of stations:
            'StWL' = [0] * gv.hws['StNo']
    ...
    def _check_gpio(self):  # tests if gpio present, else simulated mode is set
        # gv.hw['WithHW']  - if set, runs with pumps and valves and real sensors,
    ...
    def so_switch(self, state):  # set water source on or off
        if gv.hw['WithHW']:
            if state:
                # switch on
                self._set_pin(self.hwc['So']['Pin'], 1)
                gv.hw['SoStatus'] = 1
            else:
                # switch off
                self._set_pin(self.hwc['So']['Pin'], 0)
                gv.hw['SoStatus'] = 0
    ...
    def st_switch(self, index,  state):  # sets station valve on or off
        ...
        if gv.hw['WithHW']:
            if state:
                ...
                gv.hw['StStatus'][index] = 1
            else:
                # switch valve off
                self._set_pin(self.hwc['St'][index]['ValvePin'], 0)
                gv.hw['StStatus'][index] = 0
    ...
    def se_switch(self, index, state):  # set sensor on or off
        ...
        if gv.hw['WithHW']:
            if state:
                ...
                gv.hw['SeWLStatus'][index] = 1
            else:
                # switch sensor off
                self._set_pin(self.hwc['SeWL'][index]['OnOffPin'], 0)
                gv.hw['SeWLStatus'][index] = 0

    def so_level(self):  # measures water level of water in the water source
        # source sensor is always the last one
        if self.hwc['So']['Cap'] == -1:
            gv.hw['SoWL'] = 1
        else:
            gv.hw['SoWL'] = self.se_level(len(self.hwc['SeWL']) - 1)
