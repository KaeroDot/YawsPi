...
def init_cv():  # initialize dictionary with current values
    ...
    3b. StSH: list of numbers with current soil humidity of stations
    ...
    if not prg['Enabled']:
        ...
        if prg['Mode'] == 'waterlevel':
            ...
            if tmp[2]:
                ...
                return False
        if prg['Mode'] == 'soilhumidity':
            # soil humidity mode:
            tmp = prg_sh_is_water_time(index, now)
            # next run not available in soil humidity mode, string for web:
            tim = tmp[0] - arrow.now('local')
            if tim.total_seconds() < 0:
                gv.cv['PrgNR'][index] = 'Not yet dry'
            else:
                gv.cv['PrgNR'][index] = td_format(tim)
            if tmp[2]:
                log_add(isreadystr)
...
def prg_sh_is_water_time(index, now):  # checks if is watering time
    """ Checks if now is watering time for soil humidity mode program.

    Checks if all stations are dry, if so set FoundEmpty to True, if not set
    to False (also sets TimeFoundEmpty). Set to False is because if pot is
    found dry, and program waits some time, and someone waters it in between,
    than program would water even if stations are wet.
    Than checks time from last watering and if time from found empty is long
    enough.
    ...
    \return tuple: (bool, string, arrow), bool True if program is ready for
    watering; string with description why program is not ready for watering if
    so; arrow time of next watering if possible, if not return time smaller
    than input parameter
    """
    prg = gv.prg[index]
    # check if all stations in program are empty:
    ...
    allempty = True
    for st in prg['Stations']:
        # station is dry if value is lower than low threshold:
        if gv.cv['StSH'][st] > gv.hws['StData'][st]['shLowThr']:
            allempty = False
    if not allempty:
        # return time in history to represent that it is impossible to know
        # when next watering will be
        ...
        # sure FoundEmpty is reseted:
        gv.prg[index]['FoundEmpty'] = False
        return (tmp, 'some stations still not dry', False)
    else:
        if prg['FoundEmpty'] is False:
            # all stations found dry for first time, remember time:
            gv.prg[index]['TimeFoundEmpty'] = now
            # and set it was already found dry:
            gv.prg[index]['FoundEmpty'] = True
    # stations found empty, so:
    # check if time from last watering is long enough:
    tmp = prg['TimeLastRun'].shift(hours=prg['shMinDelayH'])
    if now <= tmp:
        return (tmp, 'not long enough from last watering', False)
    # check if stations are dry long enough:
    tmp = prg['TimeFoundEmpty'].shift(hours=prg['shEmptyDelayH'])
    if now < tmp:
        return (tmp, 'stations not dry long enough', False)
    # all conditions ok, program is ready for watering:
    return (now, 'ready for watering', True)
...
def save_station_shumid(index, value):  # save soil humidity of a station
    if index in range(gv.hw.StNo):
        # saving data for this station enabled?:
        if gv.hws['StData'][index]['SaveData']:
            save_station_data_line(index, str(value) + '; soil humidity (a.u.)')
    pass
...
class WebChangeStation:  # change station settings
    def __init__(self):
        self.frm = web.form.Form(  # definitions of all input fields
            ...
            web.form.Textbox(
                ...
                description='Water level low threshold (%)',
            ...
            web.form.Textbox(
                ...
                web.form.Validator('(number from 0 to 100)',
                                   lambda x: float(x) <= 100),
                description='Water level high threshold (%)',
            ...
            web.form.Textbox(
                ...
                web.form.Validator('(number from 0 to 100)',
                                   lambda x: float(x) <= 100),
                description='Soil Humidity low threshold (%)',
                title='If soil humidity sensor output is below this value, ' +
                'station is considered dry.',
            ),
            web.form.Textbox(
                'shHighThr',
                web.form.Validator('(number from 0 to 100)',
                ...
                web.form.Validator('(number from 0 to 100)',
                                   lambda x: float(x) <= 100),
                description='Soil Humidity high threshold (%)',
                title='If sensor output is above this value, ' +
                'station is considered wet.',
            ),
            web.form.Checkbox(
                'SaveData',
    ...
    def GET(self, indexstr):
        ...
        if indexstr in [str(i) for i in range(gv.hw.StNo)]:
            ...
            frm.shLowThr.value = gv.hws['StData'][index]['shLowThr'] * 100
            frm.shHighThr.value = gv.hws['StData'][index]['shHighThr'] * 100
        ...
        if 'submit' in response:
            if indexstr in [str(i) for i in range(gv.hw.StNo)]:
                ...
                if not frm.validates():  # if not validated
                    ...
                    frm.shLowThr.value = response['shLowThr']
                    frm.shHighThr.value = response['shHighThr']
                    ...
                    tmp['shLowThr'] = float(frm.shLowThr.value) / 100
                    tmp['shHighThr'] = float(frm.shHighThr.value) / 100
...
class WebChangeProgram:  # change program settings
    ...
    def GET(self, indexstr):
        ...
        if indexstr in [str(i) for i in range(len(gv.prg))]:
            ...
            frm.shMinDelayH.value = gv.prg[index]['shMinDelayH']
            frm.shEmptyDelayH.value = gv.prg[index]['shEmptyDelayH']
        ...
        if 'submit' in response:
            if indexstr in [str(i) for i in range(len(gv.prg))]:
                ...
                if not frm.validates():  # if not validated
                    ...
                    frm.shMinDelayH.value = response['shMinDelayH']
                    frm.shEmptyDelayH.value = response['shEmptyDelayH']
                    ...
                    if not (p['Mode'] == 'waterlevel'
                            or p['Mode'] == 'soilhumidity'
                            or p['Mode'] == 'weekly'
                            or p['Mode'] == 'interval'):
                        p['Mode'] == 'waterlevel'
                    p['wlMinDelayH'] = float(response['wlMinDelayH'])
                    p['wlEmptyDelayH'] = float(response['wlEmptyDelayH'])
                    p['shMinDelayH'] = float(response['shMinDelayH'])
                    p['shEmptyDelayH'] = float(response['shEmptyDelayH'])
...
class WebCheckPrograms:  # shows plan of programs for next 2 weeks
    def GET(self):
        ...
        if len(s) == 0:
            s = 'No programs exists, or only water level and soil humidity programs enabled ' + \
