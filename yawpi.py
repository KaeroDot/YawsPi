...
def prg_get_new():  # returns dict with a new program
    return {
        ...
        'Mode': 'weekly',
        'Mode': 'interval',
        # for waterlevel mode - not start sooner than (hours)
        'wlMinDelayH': 1,
        # for waterlevel mode - must be empty at least (hours)
        'wlEmptyDelayH': 1,
        # for weekly mode - start days in week (1 - Monday, 7 - Sunday):
        'calwDays': [1, 3, 5],
        # for both calendar modes - repeat during day every (hours):
        'calwRepeatH': 5,
        # for interval mode - repeat interval (days):
        'caliIntervalD': 1,
        ...
        'FoundEmpty': False,
        # first time when found all stations of the program empty (usefull only
        # for waterlevel mode, is used only if 'FoundEmpty' is True)
