...
# 12, zobrazit status na home page - plni, ceka na main loop a v kolik hodin
# nastane, meri sensory...
...
def init_gs():  # initialize dictionary with general settings:
    ...
    5. Location: city name for weather retrieval from internet \todo{not finished} XXX
    6. Logging: writing to log enabled
    7. LoggingLimit: maximal number of log lines to keep, 0 = no limit
    8. MLInterval: Main loop interval (s) - how often watering and water levels are checked.
    \param Nothing
    \return Nothing
    """
    gv.gs = {
...
def init_hws():  # initialize dictionary with hw settings:
    ...
    gv.hws contains hardware related values:
    1. StData: list of dictionaries containing:
        1. Name: Station name
        2. LowThr: lower threshold - if sensors value is below, station is considered empty
        3. HighThr: upper threshold - if sensors value is above, station is considered full
    ...
    \param Nothing
    \return Nothing
    \todo{check if SeData SaveData is not duplicit, and what is the difference to StData SaveData}
    """
    gv.hws = {
        'StData': [],
        'SoData': {
...
def prg_get_new():  # returns dict with a new program
    ...
    5. wlMinDelayH: integer, for waterlevel mode - not start sooner than (hours)
    6. wlEmptyDelayH, integer, for waterlevel mode - must be empty at least (hours)
    7. calwDays: list of integers, for weekly mode - start days in week (1 - Monday, 7 - Sunday):
    ...
    10. caliRepeatH: integer, for interval mode - repeat during day every (hours):
    ...
    15. TimeLastRun: arrow, last run of program (for a new program is set 1 day to the past - i.e. equal to caliIntervalD
    16. FoundEmpty: boolean, all stations of the program was already found as empty? (for a new program is set as not yet found empty (False))
    17. TimeFoundEmpty: arrow, first time when found all stations of the program empty (usefull only for waterlevel mode, is used only if 'FoundEmpty' is True)
    \param Nothing
...
def prg_add():  # add a new program
    ...
    new program with default values is appended to the gv.prg and gv.cv is updated.
    \param Nothing
    \return Nothing
    """
    # append new program:
...
if __name__ == "__main__":
    ...
    log_add('starting')
    # initialize hw:
    # maybe do not put hw to gv.hw and ensure web server cannot touch hardware... # XXX
    gv.hw = YawspiHW()
    if gv.hw.WithHW != 1:
